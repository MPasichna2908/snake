...
const soundPoint = new Audio("../sounds/eat.wav");
...
class Mouse{
    ...
    generateMouse(){
        let random_width= Math.floor(Math.random() * (WIDTH+1-blockSize));
        let random_height= Math.floor(Math.random() * (HEIGHT+1-blockSize));
...
class Snake{
    ...
    constructor(x,y,direction){
        ...
        let speed=200;
        this.intervalMoves=setInterval(moveSnake,speed);
...
let snake=new Snake(WIDTH/2,HEIGHT/2,'left',10);
    ...
    function isBiten(){
        ...
        let tick=true; //to have one direction change on one move
